								...
								<div id="id-message-list-navbar" class="message-navbar clearfix">
									<div class="message-bar">
										<div class="message-infobar" id="id-message-infobar">
											<span class="blue bigger-150">已发送${newsnum }封邮件</span> 
										</div> 
			...
			<div>
				<div class="form-group">
					...
					<div class="col-sm-6 col-xs-12">
						...
						<!-- <input type="text" class="col-xs-12" name="users" id="form-field-recipient" placeholder="请填写用户邮箱，多个用英文逗号隔开" />
							<i class="ace-icon fa fa-user"></i>
							 -->	
							 
							<select class="chosen-select form-control" name="users" id="form-field-select-1">
									 <c:forEach items="${emails }" var="e" >
									 <option value="${e.email }">${e.nickname } <${e.email }></option> 
									 </c:forEach>
							</select>
	...
	$(".page-content-area").ace_ajax("loadScripts", scripts, function() {
		...
		var state=1;
				...
				if(e.keyCode==13){  
					if($("#search").val()==""){
					 ...
					 state=0;
						$.ajax({ 
							dataType : "json",
							url : "${contextPath}/sys/email/search",
							type : "post",
							data : {
								name : $("#search").val()
							},
								...
								//	var newsnum=s.newsnum;
									...
									var html=""; 
									 for(var i=0;i<list.length;i++){
											...
											html+="<span class='sender' title='标题'>"+list[i].subject+"</span>";
											...
											html+=list[i].messageNoHTML.substr(0.50);	//"${fn:substring(n.content,0,50) }"	
											html+="</span>"	;	
											html+="</span>";	
											html+="</div>";	 
										 }
			...
			$("#shouye").click(function(){
				...
				$.ajax({ 
					dataType : "json",
					url : "${contextPath}/sys/email/getmail",
					type : "post",
					data : {
						pagenum : 1,
						style : "0",//首页
					...
					//	var newsnum=s.newsnum;
						...
						var html=""; 
						 for(var i=0;i<list.length;i++){
							...
							html+="<!-- #section:pages/inbox.message-list.item id= message-list邮件消息列表 -->";
							html+="<div class='message-item  message-unread'>";
							html+="<input type='hidden' id='thisid' value='"+list[i].id+"'>";
							html+="<label class='inline'>";
			...
			$("#sendEmailButton").click(function () {
			    if ($("#form-field-select-1").val() == "") {
			    	...
			    	$.ajax({
						...
						dataType : "json",
						url : "${contextPath}/sys/email/sendEmail",
						type : "post",
						data : {
			...
			//$("span[name='newsxq']").on("click", function(){ 
				...
				$(".message-container").append("<div class='message-loading-overlay'><i class='fa-spin ace-icon fa fa-spinner orange2 bigger-160'></i></div>");
					...
					$.ajax({ 
						async:false,
						dataType : "json",
						url : "${contextPath}/sys/email/showmail",
						type : "post",
						data : {
			...
			$("#delete").on("click",function(){ 
				...
				$.ajax({
					...
					dataType : "json",
					url : "${contextPath}/sys/email/delmail",
					type : "post",
					data : {
						 ids:ids
					},
