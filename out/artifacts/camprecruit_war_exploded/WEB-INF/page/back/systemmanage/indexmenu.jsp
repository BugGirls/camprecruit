...
<div class="row">
	<div class="col-xs-12">
		...
		<!-- PAGE CONTENT ENDS > -->
		 ...
		 <div style="display: none;position:absolute ;top:20%;left:30%;background-color: #fff;border: 1px solid #aaa;" id="aid" >
		  ...
		  <div style="padding: 10px;">
		  	<form id="form2" method="post" enctype="multipart/form-data" action="${contextPath}/sys/indexmenu/fileUpload"> 
		  		<input type="hidden" name="style" value="0"/>
		  		标题	&nbsp;<input type="text" name="title" id="atitle" maxlength="255"/><br><br>
				链接	&nbsp;<input type="text" name="href" id="ahref" maxlength="255"/><br><br>
				排序	&nbsp;<input type="text" name="sort" id="asort" maxlength="2"/> <br><br> 
				<span id="aimageh">
				上传图片	&nbsp;	<input type='file' name='file' id='afile'/> 
				</span> <br> 
				角色	&nbsp;<span id="aroletype">
						 <select name='roletype'>
						 	<option value="0">游客</option>
						 	<option value="1">学生</option>
							<option value="2">教练</option>
							<option value="3">营地</option>
		...
		<div style="display: none;position:absolute ;top:20%;left:30%;background-color: #fff;border: 1px solid #aaa;" id="hid" >
		  ...
		  <div style="padding: 10px;">
		  	<form id="form1" method="post" enctype="multipart/form-data" action="${contextPath}/sys/indexmenu/fileUpload">			
				<input type="hidden" name="id" id="fid"/><br><br>
				标题	&nbsp;<input type="text" name="title" id="ftitle"/><br><br>
				链接	&nbsp;<input type="text" name="href" id="fhref"/><br><br>
	...
	$('.page-content-area').ace_ajax('loadScripts',scripts,function() {
					...
					jQuery(function($) { 
							...
							jQuery(grid_selector).jqGrid({
												subGrid : false,
												url : "${contextPath}/sys/indexmenu/getTzzIndexMenu",
												datatype : "json",
												height : 650,
												colNames : [ '', 'ID', '角色','图片', '标题', '链接地址', '排序', '是否显示', '创建时间' ],
												colModel : [
												{
													name : '',
													index : '',
												...
												editurl : "${contextPath}/sys/indexmenu/operateTzzIndexMenu"
											//caption : "用户管理列表",
											//autowidth : true,
											/**
											grouping : true, 
				...
				//============================================================================			
							...
							function displayFormatter(cellvalue, options, rowdata) {
								if(cellvalue=="1")
									return "显示";
								else 
									return "不显示";
							}
							function roletypeFormatter(cellvalue, options, rowdata) {
								if(cellvalue=="0")
									return "游客";
								else if(cellvalue=="1")
									return "学生";
							...
							$("#editaaa").click(function(){
								...
								$("#fimage").html("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;点击图片可修改图片<br>图片"+image);
								 ...
								 var options = $("#froletype").find("option"); //获取select下拉框的所有值
								 for (var j = 1; j < options.length; j++) {
									 if ($(options[j]).val() == roletype) {
									 	$(options[j]).attr("selected", "selected");
									 }
								 } 
								//$("#froletype").val(roletype);
								 if(state=="显示"){
									 state=1;
								 }else{
									 state=0;
									...
									.jqGrid('navButtonAdd',pager_selector,{
												...
												onClickButton : function() {
										    	   ...
										    	   for (i = 0; i < ids.length; i++) {
													...
													var form = "<form name='csvexportform' action='${contextPath}/sys/carousel/operateTzzIndexCarousel?oper=excel' method='post'>";
													form = form
