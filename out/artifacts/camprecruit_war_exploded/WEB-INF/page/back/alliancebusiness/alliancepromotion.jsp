...
<div class="row">
	<div class="col-xs-12">
		...
		<script type="text/javascript">
	     function cancelagentpromotion(promotionid){
				$.ajax({
			        type:"post", 
			        url:"${contextPath}/sys/alliancepromotion/cancelAgentPromotion",
        ...
        $('.page-content-area').ace_ajax('loadScripts', scripts, function() {
        	...
        	jQuery(function($) {
        		...
        		jQuery(grid_selector).jqGrid({
        			subGrid : false,
        			url : "${contextPath}/sys/alliancepromotion/getAgentPromotionList?from=agent",
        			datatype : "json",
        			height : 650,
        			colNames : ['活动id','活动名称','活动价格','活动有效期至', '开始代理日期', '操作'],
        			colModel : [ {
        				name : 'promotionid',
        				index : 'promotionid',
        				label : '活动id',
...
//         				width : 200 ,
        				editable : true,
        				search : false,
        				formatter : cancelagentfromat
        				//formatter: function (value, grid, rows, state) {
          				//	 return "&nbsp;&nbsp;&nbsp;&nbsp;<button id= \"" + value + "\" style=\"background-color:#eee;border:1px solid #777; \" onclick='cancelagentpromotion(" + value + ")'>取消代理</button>";} 
        			}
					],
        ...
        //=====================================================
			...
			$('[data-rel=popover]').popover({html:true});
				...
				function cancelagentfromat(cellvalue, options, rowdata) {  
					if(rowdata.agenttype==0){
						return "&nbsp;&nbsp;&nbsp;&nbsp;<button id= \"" + cellvalue + "\" style=\"background-color:#eee;border:1px solid #777; \" onclick='cancelagentpromotion(" + cellvalue + ")'>取消代理</button>";
					}else{
						return "";
					}
				...
				function cancelFormatter (cellvalue, options, cell) {
 					 return "<button data-toggle='modal' onclick='javascript:$(\"#modal-table\").modal(\"toggle\");$(\"#promotionid\").val(\"" + cellvalue +"\");'>取消代理</button>";
 					 } 
...
//     					}
				...
				$("#addaaa").click(function(){
					window.location.href = "${contextPath}/sys/sysuser/home#page/notagentpromotion";
					//$("#modal-table").modal("toggle");
				});
