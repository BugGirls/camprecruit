        	...
        	jQuery(function($) {
        		...
        		$("#submit").click(function(){
        			if($("#f3").val()!=""){ 
        				 ...
        				 $.ajax({
      	 	      		   type:"post",
      	        		   url:"${contextPath}/sys/tixikucp/searchp?txkid=${txkid}",
      	        		   dataType:"json", 
      	       		  	   data:{"fid":$("#f3").val()},
        		...
        		//添加 ${contextPath}/sys/tixikucourse/operateTzztc?txkdid=${txkid}
        		$("#courseli").on("click",".jia",function(){ 
        			var ft = confirm("确定添加吗？"); 
        			var productid = $(this).parent().prop("id"); 
        			if(ft){ 
        			 	$.ajax({
    	 	      		   type:"post",
    	        		   url:" ${contextPath}/sys/tixikucp/operateTzztp?txkid=${txkid}",
    	        		   dataType:"json", 
    	       		  	   data:{"oper":"add","productcn":productid },
    	       		  	   success:function(data){  
        		...
        		jQuery(grid_selector).jqGrid({
        			subGrid : false,
        			url : "${contextPath}/sys/tixikucp/getTzztp?txkid=${txkid}",
        			datatype : "json",
        			height : 400,
        			colNames : ['', 'ID','产品名称','创建时间'],
        			colModel : [ {
        				name : '',
        				index : '',
        				width : 100,
        				...
        				editable : true,
        				editoptions :{ 
        					dataUrl : "${contextPath}/sys/tixikucp/getproduct"
        				},
        				edittype : "select",
        			...
        			editurl : "${contextPath}/sys/tixikucp/operateTzztp?txkid=${txkid}"
        			//caption : "用户管理列表",
        			//autowidth : true,
        			/**
        			grouping : true, 
				...
				jQuery(grid_selector).jqGrid('navButtonAdd', pager_selector,{
			       ...
			       onClickButton : function () { 
			    	   ...
			    	   var form = "<form name='csvexportform' action='${contextPath}/sys/tixikucp/operateTzztp?txkid='${txkid}'&oper=excel' method='post'>";
