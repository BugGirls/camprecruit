...
<div class="row">
	...
	<a href="javascript:history.go(-1);">返回上一页</a>
		...
		<div style="position:absolute; ;top:727px;left:162px; padding-right: 100px;">
 		...
 		<div style="display: none;position:absolute ;z-index:999;top:0;left:0;background-color: #fff;border: 1px solid #aaa;width: 600px;"" id="aid" >
		  <div style="border:1px solid #aaa ; background-color:#eee; color:#23b820;font-size:20px ;padding:5px ;text-align:center;">
			 ...
			 <input type="button"  style="float:left;"name="aclose"class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn btn-sm" value="取消">
		  	...
		  	<form id="form2" method="post" enctype="multipart/form-data" action="${contextPath}/sys/comboPrice/savaOrUpdateDatePrice">  
			<input type="hidden" id="addContent" name="addContent"/>
			<input type="hidden" name="cid" value="${cid }"/>
			<div style="float:left;"> 
		  		日期	&nbsp;<input type="text" name="date" id="adate" class="date"/><br><br>
				价格	&nbsp;&nbsp;&nbsp;<input type="text" name="price" id="aprice" maxlength="10"/><br><br>
			 	每日限额	&nbsp;<input type="text" name="daylimit" id="adaylimit" /><br><br>
		...
		<div style="display: none;position:absolute ; z-index:999; top:0;left:0;background-color: #fff;border: 1px solid #aaa;" id="hid" >
		   <div style="border:1px solid #aaa ; background-color:#eee; color:#23b820;font-size:20px ;padding:5px ;text-align:center;">
			 ...
			 <input type="button"  style="float:left;"name="fclose"class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn btn-sm" value="取消">
		  	...
		  	<form id="form1" method="post" enctype="multipart/form-data" action="${contextPath}/sys/comboPrice/savaOrUpdateDatePrice">			
				<input type="hidden" name="id" id="fid"/><br><br>
				<input type="hidden" name="cid" value="${cid }"/>
				<div style="float:left">
				日期	&nbsp;<input type="text" name="date" id="fdate" class="date"/><br><br>
				价格	&nbsp;&nbsp;&nbsp;<input type="text" name="price" id="fprice" maxlength="10"/><br><br>
			 	每日限额	&nbsp;<input type="text" name="daylimit" id="fdaylimit" /><br><br>
        ...
        $('.page-content-area').ace_ajax('loadScripts', scripts, function() {
        	...
        	jQuery(function($) {
        		...
        		jQuery(grid_selector).jqGrid({
        			subGrid : false,
        			url : "${contextPath}/sys/comboPrice/getComboDatePriceList?cid=${cid}",
        			datatype : "json",
        			height : 650,
        			colNames : ['', 'ID', '日期','价格','限额','设置时间'],
        			colModel : [ {
        				name : '',
        				index : '',
        				width : 60,
        			...
        			editurl : "${contextPath}/sys/comboPrice/operateDatePrice"
        			//caption : "用户管理列表",
        			//autowidth : true,
        			/**
        			grouping : true, 
...
// 					if($("#fdate").val()!=""&&!$("#fdate").val().match(/^((?:19|20)\d\d)-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])$/)&&!$("#fdate").val().match(/^((?:19|20)\d\d)\/(0[1-9]|1[012])\/(0[1-9]|[12][0-9]|3[01])$/)) { 
...
// 					if($("#adate").val()!=""&&!$("#adate").val().match(/^((?:19|20)\d\d)-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])$/)&&!$("#adate").val().match(/^((?:19|20)\d\d)\/(0[1-9]|1[012])\/(0[1-9]|[12][0-9]|3[01])$/)) { 
// 		 				alert("日期格式不正确，正确格式例如'2009-03-09'"); 
// 		 				return;
// 		 			}
					var regz = new RegExp("^[0-9]*[1-9][0-9]*$");
					var regk=new RegExp("^[0-9]+(.[0-9]{1,2})?$");   
    			    if($("#aprice").val()!=""&&$("#fprice").val()!="0"&&!regk.test($("#aprice").val())){  
    			        alert("价格必须是两位小数数字格式!");  
    			        return ;
    			    }
