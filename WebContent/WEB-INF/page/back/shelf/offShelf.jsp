		...
		<div style="padding: 10px;">
			...
			<form id="form1" method="get" action="#">  
				<table class="table table-bordered">
					...
					<tr>
						<td>下架单号</td>
						<td><input type="text" value="${no }" name="offShelfNo" id="offShelfNo" style="width:220px;" readonly="readonly"/></td>
						<td>操作人员</td>
...
function addDetailRow(){
	...
	var td1 = "<td>"+num+"<input type='hidden' id='seqence' name='seqence' value="+num+"><input type='hidden' id='onShelfId' name='onShelfId' value=''><input type='hidden' id='onShelf_no' name='onShelf_no' value=''><input type='hidden' id='productWarehouseId' name='productWarehouseId' value=''><input type='hidden' id='intoWarehouseRecordNo' name='intoWarehouseRecordNo' value=''></td>";
...
function changeProductNo(num){
	...
	$.ajax({
        type:"post", 
        url:"${contextPath}/sys/productShelf/findByNoAndAllianceId",
        data:{"productNo":productNo,"allianceId":allianceId},
   		dataType:"json", 
   		async: false,//非异步
       	success:function(data){   
       		if(null != data && '' != data){
       			$("#detail"+num).find("#productWarehouseId").val(data.productWarehouseId);
       			$("#detail"+num).find("#onShelfId").val(data.id);
       			$("#detail"+num).find("#intoWarehouseRecordNo").val(data.intoWarehouseRecordNo);
       			$("#detail"+num).find("#onShelf_no").val(data.onShelf_no);
		...
		error:function(){
			...
			$("#detail"+num).find("#onShelfId").val("");
			$("#detail"+num).find("#onShelf_no").val("");
...
function changeNum(currNum){
	var onShelfId = $("#onShelfId").val();
	var onshelfNum = Number($("#detail"+currNum).find("#num").val());
	$.ajax({
        type:"post", 
        url:"${contextPath}/sys/productShelf/findByProductWarehouseId",
        data:{"onShelfId":onShelfId},
   		dataType:"json", 
   		async: false,//非异步
       	success:function(data){   
       		if(null != data && '' != data){
...
$("input[name='aup']").click(function(){  
	...
	$.ajax({
	url:"${contextPath}/sys/productShelf/offProductShelf",
	type:"POST",
	data:{"detail":JSON.stringify(detailArray), "creater":creater, "createrNo":createrNo, "allianceId":allianceId, "offShelfNo":offShelfNo},
	async: false,//非异步
	dataType : "json",
	success:function(data){
		location.href = "${contextPath}/sys/sysuser/home";
		location.href = "${contextPath}/sys/sysuser/home#page/checkShelves";
	},
	error:function(){
		location.href = "${contextPath}/sys/sysuser/home";
		location.href = "${contextPath}/sys/sysuser/home#page/checkShelves";
