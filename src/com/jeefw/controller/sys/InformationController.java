	...
	public void getInformation(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (StringUtils.isNotBlank(filters)) {
			...
			for (int i = 0; i < jsonArray.size(); i++) {
				...
				if (result.getString("field").equals("author") && result.getString("op").equals("cn")) {
					information.set$like_author(result.getString("data"));
				}
			}
	...
	public void operateInformation(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (oper.equals("del")) {
			...
			if (StringUtils.isBlank(title)) {
				...
				//width="242" height="149"
				String img =image.split("/>")[0];
				img+= " width='242px' height='149px' />";
				entity.setImage(img);
