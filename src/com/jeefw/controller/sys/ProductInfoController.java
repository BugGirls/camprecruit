...
public class ProductInfoController extends JavaEEFrameworkBaseController<ProductInfoType> implements Constant {
		...
		if (StringUtils.isNotBlank(filters)) {
			...
			for (int i = 0; i < jsonArray.size(); i++) {
				...
				if (result.getString("field").equals("brand") && result.getString("op").equals("cn")) {
					productInfo.set$like_brand(result.getString("data"));
				}
				if (result.getString("field").equals("barCode") && result.getString("op").equals("cn")) {
					productInfo.set$like_barCode(result.getString("data"));
				}
				if (result.getString("field").equals("bigCategoryName") && result.getString("op").equals("cn")) {
					productInfo.set$like_bigCategoryName(result.getString("data"));
				}
				if (result.getString("field").equals("smallCategoryName") && result.getString("op").equals("cn")) {
					productInfo.set$like_smallCategoryName(result.getString("data"));
				}
			}
			if (((String) jsonObject.get("groupOp")).equalsIgnoreCase("OR")) {
				productInfo.setFlag("OR");
			} else {
				productInfo.setFlag("AND");
			}
		}
		productInfo.setFirstResult((firstResult - 1) * maxResults);
		productInfo.setMaxResults(maxResults);
		Map<String, String> sortedCondition = new HashMap<String, String>();
		sortedCondition.put(sortedObject, sortedValue);
		productInfo.setSortedConditions(sortedCondition);
		QueryResult<ProductInfo> queryResult = productInfoService.doPaginationQuery(productInfo);
		JqGridPageView<ProductInfo> dictListView = new JqGridPageView<ProductInfo>();
		dictListView.setMaxResults(maxResults);
		List<ProductInfo> dictWithSubList = productInfoService.queryProductInfoWithSubList(queryResult.getResultList());
