...
public class SendMail {
	 ...
	 * Just do it as this
	 */
	public  String send( Mail mail,String smtp, String pwd ) {
		...
		if (themail.setBody(mail.getMessage()) == false)
			return "0";
		// 收件人邮箱
		if (themail.setTo(mail.getRecipient()) == false)
			return "0";
		// 发件人邮箱
		if (themail.setFrom(mail.getSender()) == false)
			return "0";
		// 设置附件
		// if (themail.addFileAffix("#######") == false)
		// return; // 附件在本地机子上的绝对路径
		themail.setNamePass(mail.getSender(),pwd); // 用户名与密码
		if (themail.sendout(mail) == false)
			return "0";
		else {
			return "1";
		}
